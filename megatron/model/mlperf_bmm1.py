...
class Bmm1Function(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        mhalib.FastBmm1Dgrad2(batch2.flatten().contiguous(), grad_output.flatten().contiguous(),
                              ...
                              ctx.stream, ctx.sync)
        mhalib.FastBmm1Dgrad1(batch1.flatten().contiguous(), grad_output.flatten().contiguous(),
                              grad_batch2.flatten().contiguous(), batch, seqlen, heads, embed, ctx.scale, False,
                              ctx.stream, ctx.sync)
...
class Bmm1(torch.nn.Module):
    ...
    def forward(self, batch1, batch2, batch, seqlen):
        return Bmm1Function.apply(batch1, batch2, seqlen, batch, self.maxseqlen, self.heads, self.embed, self.scale,
...
class Bmm1StridedFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output, grad_mixed):
        ...
        mhalib.FastBmm1Dgrad2(mixed, grad_output, grad_mixed, batch, seqlen, heads, embed, ctx.scale, True, ctx.stream,
                              ctx.sync)
        if ctx.timers: ctx.timers['stop_dgrad'].record()
        if ctx.timers: ctx.timers['start_wgrad'].record()
        mhalib.FastBmm1Dgrad1(mixed, grad_output, grad_mixed, batch, seqlen, heads, embed, ctx.scale, True, ctx.stream,
                              ctx.sync)
...
class Bmm1Strided(torch.nn.Module):
    ...
    def forward(self, mixed, batch, seqlen):
        return Bmm1StridedFunction.apply(mixed, seqlen, batch, self.maxseqlen, self.heads, self.embed, self.scale,
