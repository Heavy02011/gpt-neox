...
class Bmm2Function(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        mhalib.FastBmm2Dgrad2(grad_output, batch1, grad_batch2, batch, seqlen, heads, embed, False, False, ctx.stream,
                              ctx.sync)
...
class Bmm2(torch.nn.Module):
    ...
    def forward(self, batch1, batch2, batch, seqlen):
        return Bmm2Function.apply(batch1, batch2, seqlen, batch, self.maxseqlen, self.heads, self.embed, self.stream,
                                  self.sync)
...
class Bmm2StridedFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        mhalib.FastBmm2Dgrad2(grad_output, batch1, grad_mixed, batch, seqlen, heads, embed, False, True, ctx.stream,
                              ctx.sync)
...
class Bmm2Strided(torch.nn.Module):
    ...
    def forward(self, batch1, mixed, batch, seqlen):
        return Bmm2StridedFunction.apply(batch1, mixed, seqlen, batch, self.maxseqlen, self.heads, self.embed,
